...
- **记录**:点数字盘记录每一局开出的号,或粘贴历史批量导入;支持欧式(0–36)/美式(0,00,1–36)
- **频率统计**:每个数字的出现次数、热力条;红黑/单双/大小/打/列等投注区频率
- **偏差检测(核心)**:
  - 整体**卡方检验** χ²:判断这台轮盘整体是否偏离"完全随机"
  - 单号 **z 值**:衡量某个号偏离期望(1/格数)的程度,|z|≥3 才算有统计意义
  - **数据充分度提示**:样本不足时明确告诉你"现在看到的热门号只是噪声"
- **下一局概率估计(诚实版)**:数据不足时直说"无法预测";数据充分且检验显著时,列出统计上偏多的号,并提醒优势有限
